...
func (sb *StateBuffer) Clean() {
  ...
  if sb.dirtySeq < sb.currentSeq {
    ...
    for i := 0; i < diff; i++ {
      sb.past[dirtyIdx].Angle = WrapAngle(sb.past[cleanIdx].Angle.Add(sb.past[dirtyIdx].AngleDelta))
  ...
  if sb.dirtySeq == sb.currentSeq {
    sb.current.Angle = WrapAngle(sb.past[sb.pastHead].Angle.Add(sb.current.AngleDelta))
  ...
  if sb.dirtySeq == sb.currentSeq + 1 {
    sb.future[sb.futureHead].Angle = WrapAngle(sb.current.Angle.Add(sb.future[sb.futureHead].AngleDelta))
  ...
  if sb.dirtySeq > sb.currentSeq {
    ...
    for i := 0; i < sb.size - 1; i++ {
      sb.future[dirtyIdx].Angle = WrapAngle(sb.future[cleanIdx].Angle.Add(sb.future[dirtyIdx].AngleDelta))
